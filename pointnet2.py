...
def sample_and_group_abs(npoint, radius, nsample, xyz, points):
    """
    Input:
        npoint:
        radius:
...
class PointNetSetAbstractionAbs(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all):
        super(PointNetSetAbstractionAbs, self).__init__()
        self.npoint = npoint
        self.radius = radius
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all:
            ...
            new_xyz, new_points = sample_and_group_abs(self.npoint, self.radius, self.nsample, xyz, points)
...
class PointNetSetAbstractionMsgAbs(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        super(PointNetSetAbstractionMsgAbs, self).__init__()
        self.npoint = npoint
...
class PointNetFeaturePropagationMSG(nn.Module):
    def __init__(self, in_channel, mlp):
        super(PointNetFeaturePropagationMSG, self).__init__()
        self.mlp_convs = nn.ModuleList()
        self.mlp_bns = nn.ModuleList()
        last_channel = in_channel
        for out_channel in mlp:
...
class PointNet2PointFeatureAbs(nn.Module):
    def __init__(self):
        super(PointNet2PointFeatureAbs, self).__init__()
        self.sa1 = PointNetSetAbstractionAbs(512, 0.2, 64, 3, [64, 64, 128], False)
        self.sa2 = PointNetSetAbstractionAbs(128, 0.4, 64, 128 + 3, [128, 128, 256], False)
        self.sa3 = PointNetSetAbstractionAbs(None, None, None, 256 + 3, [256, 256, 1024], True)
...
class PointNet2Scores(nn.Module):
    def __init__(self):
        super(PointNet2Scores, self).__init__()
        self.sa1 = PointNetSetAbstraction(512, 0.2, 32, 3, [64, 64, 128], False)
        self.sa2 = PointNetSetAbstraction(128, 0.4, 64, 128 + 3, [128,128,256], False)
        self.sa3 = PointNetSetAbstraction(None, None, None, 256 + 3, [256, 512, 1024], True)
        self.fc1 = nn.Linear(1024, 512)
    ...
    def forward(self, xyz):
        ...
        x = self.fc3(x)
        x = self.softmax(x, dim=-1)[:, 0]
        return x

class PointNet2PointFeature(nn.Module):
    def __init__(self):
        super(PointNet2PointFeature, self).__init__()
...
class PointNet2PointFeatureStable(nn.Module):
    def __init__(self):
        super(PointNet2PointFeatureStable, self).__init__()
        self.sa1 = PointNetSetAbstraction(512, 0.2, 64, 3, [64, 64, 128], False)
        self.sa2 = PointNetSetAbstraction(128, 0.4, 64, 128 + 3, [128, 128, 256], False)
        self.sa3 = PointNetSetAbstraction(None, None, None, 256 + 3, [256, 256, 1024], True)
        self.fp3 = PointNetFeaturePropagation(1280, [256, 256])
