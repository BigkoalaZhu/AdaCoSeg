...
for i, batch in enumerate(dataTestLoad):
    ...
    trainMask = batch[1].numpy().astype(int)
    trainMask = np.squeeze(trainMask, axis=0)
    ...
    draw3DPoints.savePointCloudColorSeg(shapeNp, segMask, segmentationK, "refineTraining/test-" + str(i) + ".ply")
    draw3DPoints.savePointCloudColorSeg(shapeNp, trainMask-1, np.max(trainMask), "refineTraining/train-" + str(i) + ".ply")
